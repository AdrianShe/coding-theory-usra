...
int main(int argc, char* argv[]){
    int n_end = 8;
    int z_end = 4;
    for (double z = 3; z <= z_end; z+=0.05){
        string name_n = std::to_string(z);
        ofstream n_output (string("rhombus_perc_z = " + name_n +".txt").c_str());
        n_output << left << setw(5) << "n" << setw(5) << "a" << setw(30) << "z" << setw(30) << "prob" << endl;
        cout << left << setw(5) << "n" << setw(5) << "a" << setw(30) << "z" << setw(30) << "prob" << endl;
        for (int n = n_end; n >= 0; n--){
            std::vector<long> seqs = generate_linear_one_dim_sequences(2*n+1);
            std::vector<int> counts = generate_counts(seqs);
            for (int a = 0; a < 4*n; a++){
                Eigen::VectorXd vec = generate_init(z,seqs, counts);
                double prob_2 = probability_rhombus_imp_omit(a, z, n, counts, seqs);
                cout << left << setw(5) << n << setw(5) << a << setw(30) << z << setw(30) << setprecision(16) << prob_2 << endl; 
                n_output << left << setw(5) << n << setw(5) << a << setw(30) << z << setw(30) << setprecision(16) << prob_2 << endl; 
            }
        }
    }
}
